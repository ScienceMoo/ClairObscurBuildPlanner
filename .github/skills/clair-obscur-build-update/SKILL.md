...
- [src/data/weaponsCatalog.js](../../../src/data/weaponsCatalog.js) — generated from `public/weapons.csv`: 106 weapons (`character, name, power, element, vitality, defense, agility, luck, image`), one row per weapon, NOT shared across characters (filter by `character`). Images live in `public/weapons/*.png` (real PNGs with alpha — the wiki CDN serves WebP regardless of extension/Accept header; convert with `sips -s format png <file> --out <file>` on macOS if regenerating).
